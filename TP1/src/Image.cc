...
BOOL XOU(BOOL i,BOOL j) {if (i<j) return(j-i); else return (i-j);}
...
template <> Image<BOOL> Image<BOOL>::operator& (const Image<BOOL>& p) const
  ...
  for (int j=0;j<L;j++)
    for (int i=0;i<W;i++){
      if (I[index]<p.I[index]) value=I[index]; else value=p.I[index];
      q.I.Ecrit_info(index++,value);
    } 
  return q;
   }
...
template <> Image<BOOL> Image<BOOL>::operator! () const
{
  ...
  for (int j=0;j<L;j++)
    for (int i=0;i<W;i++) { 
      q.I.Ecrit_info(index,255-I[index]);
      index++;
    }
  return q;
}
...
template <> Image<BOOL> Image<BOOL>::operator^ (const Image<BOOL>& p) const
{
  ...
  for (int j=0;j<L;j++)
    for (int i=0;i<W;i++) {
      if (I[index]>p.I[index]) value=I[index]-p.I[index]; else value=p.I[index]-I[index];
      q.I.Ecrit_info(index++,value);
    }
  return q;
}
